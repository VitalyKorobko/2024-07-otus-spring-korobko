...
public class BookControllerTest {
    ...
    @Test
    @DisplayName("должен возвращать ErrorResponse, если сработал CircuitBreaker при попытке получить список всех книг")
    void shouldReturnErrorResponseIfCircuitBreakerOpen() throws Exception {
        when(bookService.findAll()).thenThrow(new RuntimeException());
        mvc.perform(get("/api/v1/book"))
    ...
    @Test
    @DisplayName("должен возвращать ErrorResponse, если сработал CircuitBreaker при запросе книги по id")
    void shouldReturnErrorResponseIfCircuitBreakerOpenByAnyBook() throws Exception {
        when(bookService.findById(1)).thenThrow(new RuntimeException());
        mvc.perform(get("/api/v1/book/1"))
                .andExpect(status().isInternalServerError())
                .andExpect(content().json(mapper.writeValueAsString(new ErrorResponse(NOT_AVAILABLE_MESSAGE))));
    }
    ...
    void shouldReturnErrorResponseIfBookDtoWebByNewBookIsNotCorrect() throws Exception {
        ...
        mvc.perform(post("/api/v1/book").contentType(APPLICATION_JSON)
                        .content(mapper.writeValueAsString(new BookDtoWeb(0, title, authorName, setGenres, ""))))
                .andExpect(status().isInternalServerError())
                .andExpect(content().json(mapper.writeValueAsString(new ErrorResponse(NOT_AVAILABLE_MESSAGE))));
    }

    @Test
    @DisplayName("должен возвращать ErrorResponse, если сработал CircuitBreaker при сохранении новой книги")
    void shouldReturnErrorResponseIfCircuitBreakerOpenBySaveNewBook() throws Exception {
        var bookDto = new BookDto(
                1,
                "title_111",
                new AuthorDto(1, "author_111"),
        ...
        var bookDtoWeb = bookMapper.toBookDtoWeb(bookDto, null);
        when(bookService.insert(bookDtoWeb.getTitle(), bookDtoWeb.getAuthorFullName(), bookDtoWeb.getSetGenreNames()))
    ...
    void shouldReturnErrorResponseIfBookDtoWebByExistBookIsNotCorrect() throws Exception {
        ...
        mvc.perform(patch("/api/v1/book/1").contentType(APPLICATION_JSON)
                        .content(mapper.writeValueAsString(new BookDtoWeb(1, title, authorName, setGenres, ""))))
                .andExpect(status().isInternalServerError())
                .andExpect(content().json(mapper.writeValueAsString(new ErrorResponse(NOT_AVAILABLE_MESSAGE))));
    }

    @Test
    @DisplayName("должен возвращать ErrorResponse, если сработал CircuitBreaker при обновлении книги")
    void shouldReturnErrorResponseIfCircuitBreakerOpenByUpdateBook() throws Exception {
        var bookDto = new BookDto(
                1,
                "title_111",
                new AuthorDto(1, "author_111"),
        ...
        var bookDtoWeb = bookMapper.toBookDtoWeb(bookDto, null);
        when(bookService.update(1, bookDtoWeb.getTitle(), bookDtoWeb.getAuthorFullName(), bookDtoWeb.getSetGenreNames()))
                .thenThrow(new RuntimeException());
        String requestBody = mapper.writeValueAsString(bookDtoWeb);
