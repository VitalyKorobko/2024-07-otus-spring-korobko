...
public class CommentControllerTest {
    ...
    @Test
    @DisplayName("должен возвращать ErrorResponse, если сработал CircuitBreaker при попытке получить список комментариев по id книги")
    void shouldReturnErrorResponseIfCircuitBreakerOpen() throws Exception {
        when(commentService.findAllCommentsByBookId(1)).thenThrow(new RuntimeException());
        mvc.perform(get("/api/v1/comment").param("book_id", "1"))
    ...
    @Test
    @DisplayName("должен возвращать ErrorResponse, если сработал CircuitBreaker при запросе комментария по id")
    void shouldReturnErrorResponseIfCircuitBreakerOpenByAnyComment() throws Exception {
        when(commentService.findById(1)).thenThrow(new RuntimeException());
        mvc.perform(get("/api/v1/comment/1"))
                .andExpect(status().isInternalServerError())
                .andExpect(content().json(mapper.writeValueAsString(new ErrorResponse(NOT_AVAILABLE_MESSAGE))));
    }
    ...
    void shouldReturnErrorResponseIfCommentDtoByNewBookIsNotCorrect() throws Exception {
        ...
        mvc.perform(post("/api/v1/comment").contentType(APPLICATION_JSON)
                        .content(mapper.writeValueAsString(new CommentDtoWeb(0, text, bookId, ""))))
                .andExpect(status().isInternalServerError())
                .andExpect(content().json(mapper.writeValueAsString(new ErrorResponse(NOT_AVAILABLE_MESSAGE))));
    }

    @Test
    @DisplayName("должен возвращать ErrorResponse, если сработал CircuitBreaker при сохранении нового комментария")
    void shouldReturnErrorResponseIfCircuitBreakerOpenBySaveNewComment() throws Exception {
        var commentDto = new CommentDto(1, "text_1", 1);

        when(commentService.insert(commentDto.getText(), commentDto.getBookId()))
                .thenThrow(new RuntimeException());

        String requestBody = mapper.writeValueAsString(commentDto);
    ...
    void shouldReturnErrorResponseIfBookDtoWebByExistBookIsNotCorrect() throws Exception {
        ...
        mvc.perform(patch("/api/v1/comment/1").contentType(APPLICATION_JSON)
                        .content(mapper.writeValueAsString(new CommentDtoWeb(commentId, text, bookId, ""))))
                .andExpect(status().isInternalServerError())
                .andExpect(content().json(mapper.writeValueAsString(new ErrorResponse(NOT_AVAILABLE_MESSAGE))));
    }

    @Test
    @DisplayName("должен возвращать ErrorResponse, если сработал CircuitBreaker при обновлении комментария")
    void shouldReturnErrorResponseIfCircuitBreakerOpenByUpdateComment() throws Exception {
        var commentDto = new CommentDto(1, "text_1", 1);
        var expectedCommentDto = new CommentDto(1, "text_2", 2);
